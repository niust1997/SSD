...
use vars qw ($opt_h $opt_w $opt_W $opt_d $opt_D $opt_R $opt_r $opt_S $opt_s $opt_l $opt_c $opt_x $opt_E);
...
use Getopt::Std;
getopts ('nhdDwWox1mvec:R:r:S:s:t:T:g:p:P:a:A:u:l:y:Y:z:Z:E:J');
...
sub evaluate {
    ...
    my ($ref_data, $sys_data, $uem_data, $error_list) = @_;
    ...
    sd_performance_analysis ($scores{SPEAKER}, \%spkr_subtypes, $error_list)
	if $scores{SPEAKER};
}
...
sub sd_performance_analysis {
    ...
    my ($scores, $subtypes, $error_list) = @_;
...
#accumulate statistics
	...
	#	my $errorFile = "error_list.txt";
	#	my $high_error_file = "high_error_file_list.txt";
		#unlink $errorFile
		#	if -e $errorFile;
		#unlink $high_error_file
		#	if -e $high_error_file;
		  my $high_error_list = $error_list;
    my $position = index($error_list, ".");
    $high_error_list =  substr($error_list, 0, $position);
    $high_error_list = "high_" . $high_error_list . ".txt";
	
	unlink $error_list
		if -e $error_list;
	unlink $high_error_list
		if -e $high_error_list;
    foreach my $condition (sort keys %cum_scores) {
	print_sd_scores ($condition, $cum_scores{$condition}, $error_list) if $condition !~ /ALL/;
    }
    print_sd_scores ("ALL", $cum_scores{ALL}, $error_list);
}
...
sub print_sd_scores {
    ...
    my ($condition, $scores, $error_list) = @_;
    my $speaker_error = 100*$scores->{SPEAKER_ERROR}/$scores->{SCORED_SPEAKER};
    
    my $high_error_list = $error_list;
    my $position = rindex($error_list, ".");
    $high_error_list =  substr($error_list, 0, $position);
    $high_error_list = $high_error_list . "_high.txt";
 #   open(FILE_highError, ">>high_error_file_list.txt");
 #   open(FILE_error, ">>error_list.txt");
 #			print $error_list, "\n";
 	#		print $high_error_list, "\n";
		 open(FILE_highError, ">>$high_error_list");
		 open(FILE_error, ">>$error_list");
    if($speaker_error  > 10)
    {
    	printf FILE_highError "$condition\n";
			printf FILE_highError  "     SPEAKER ERROR =%6.2f percent of scored speaker words\n", $speaker_error;
		}
		printf FILE_error "$condition\n";
		printf FILE_error "    SCORED SPEAKER TIME =%8.2f secs (%5.2f percent)\n",
        $scores->{SCORED_SPEAKER}, 100*$scores->{SCORED_SPEAKER}/$scores->{SCORED_SPEECH};
    printf FILE_error "    MISSED SPEAKER TIME =%8.2f secs (%5.2f percent)\n",
        $scores->{MISSED_SPEAKER}, 100*$scores->{MISSED_SPEAKER}/$scores->{SCORED_SPEAKER};
    printf FILE_error "    FALARM SPEAKER TIME =%8.2f secs (%5.2f percent)\n",
        $scores->{FALARM_SPEAKER}, 100*$scores->{FALARM_SPEAKER}/$scores->{SCORED_SPEAKER};
    printf FILE_error "     SPEAKER ERROR TIME =%8.2f secs (%5.2f percent)\n",
        $scores->{SPEAKER_ERROR}, 100*$scores->{SPEAKER_ERROR}/$scores->{SCORED_SPEAKER};
		
		close(FILE_highError);
		close(FILE_error);
